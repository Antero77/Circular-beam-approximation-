...
for i in range(number_dot_KS):
    aperture=0.003+(1.7*longterm-0.001)*i/number_dot_KS
    l = 2000
    g = -15
    Cn2 = 10 ** g
    ...
    for i in t:
        ...
        tmp=res.cdf.evaluate(i)
        if tmp > maxY_sim:
            maxY_sim=tmp
            maxX_sim=i

        if pdt_circ_sim[count-1] > maxY_circsim:
            maxY_circsim = pdt_circ_sim[count-1]
            maxX_circsim = i

        if pdt_circ_analy[count-1] > maxY_circanaly:
            maxY_circanaly = pdt_circ_analy[count-1]
            maxX_circanaly = i

        if pdt_anc_sim[count-1] > maxY_ancsim:
            maxY_ancsim = pdt_anc_sim[count-1]
            maxX_ancsim = i

        if pdt_anc_analy[count-1] > maxY_ancanaly:
            maxY_ancanaly = pdt_anc_analy[count-1]
            maxX_ancanaly = i



    """
    dist_circSim.append( np.sqrt((maxY_circsim/maxY_sim-1)**2+(maxX_circsim/maxX_sim-1)**2) )
    dist_circanaly.append( np.sqrt((maxY_circanaly/maxY_sim-1)**2+(maxX_circanaly/maxX_sim-1)**2) )
    dist_ancSim.append( np.sqrt((maxY_ancsim/maxY_sim-1)**2+(maxX_ancsim/maxX_sim-1)**2) )
    dist_ancanaly.append( np.sqrt((maxY_ancanaly/maxY_sim-1)**2+(maxX_ancanaly/maxX_sim-1)**2) )
    frac.append(quick_channel.pupil.radius/longterm)
    """
    dist_circSim.append( (maxX_circsim-maxX_sim) )
    dist_circanaly.append( (maxX_circanaly-maxX_sim) )
    dist_ancSim.append( (maxX_ancsim-maxX_sim) )
    dist_ancanaly.append( (maxX_ancanaly-maxX_sim) )
    frac.append(quick_channel.pupil.radius/longterm)
...
X_Y_Spline = sp.interpolate.make_interp_spline(frac, dist_circSim)
...
X_Y1_Spline = sp.interpolate.make_interp_spline(frac, dist_circanaly)
...
X_Y2_Spline = sp.interpolate.make_interp_spline(frac, dist_ancSim)
...
X_Y3_Spline = sp.interpolate.make_interp_spline(frac, dist_ancanaly)
...
"""
ax.grid()
ax.set(xlabel=r'Normalized aperture radius $a/W_{LT}$', ylabel='Cartesian distance between peaks')
plt.savefig("PDT_dist.pdf", **save_kwargs)
